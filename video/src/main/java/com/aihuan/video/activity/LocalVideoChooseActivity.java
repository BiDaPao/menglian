...
public class LocalVideoChooseActivity extends AbsActivity implements OnItemClickListener<VideoChooseBean> {
        ...
        mVideoLocalUtil.getLocalVideoList(new CommonCallback<List<VideoChooseBean>>() {
            ...
            public void callback(List<VideoChooseBean> videoList) {
                ...
                if (mRecyclerView != null) {
                    ...
                    adapter.setOnItemClickListener(LocalVideoChooseActivity.this);
...
//        setResult(RESULT_OK, intent);
        if (CommonAppConfig.getInstance().getVideoPublishType()==Constants.DYNAMIC_VIDEO_PUBLISH){
            EventBus.getDefault().post(new ChooseVideoEvent(bean.getVideoPath()));
        }else {
            VideoPublishActivity.forward(mContext, bean.getVideoPath(), Constants.VIDEO_SAVE_PUB,  0);
        }
        finish();
    }
