package com.aihuan.one.presenter;
...
import com.aihuan.one.event.LinkMicTxAccEvent;
import com.aihuan.one.event.LinkMicTxMixStreamEvent;
...
import com.aihuan.one.views.AbsLiveLinkMicPlayViewHolder;
import com.aihuan.one.views.AbsLiveLinkMicPushViewHolder;
import com.aihuan.one.views.LiveLinkMicPlayTxViewHolder;
import com.aihuan.one.views.LiveLinkMicPushTxViewHolder;
...
public class LiveLinkMicPresenter implements View.OnClickListener {
    ...
    public LiveLinkMicPresenter(Context context,  boolean isAnchor, int liveSdk, View root) {
        mContext = context;
        mRoot = root;
        mIsAnchor = isAnchor;
        mLiveSdk = liveSdk;
...
//        if (isLinkMic() || ((ChatBaseActivity) mContext).isLinkMicAnchor()||((ChatBaseActivity) mContext).isGamePlaying()) {
...
//        }
    ...
    public void onAnchorAcceptLinkMic() {
        if (!mIsAnchor) {
            ...
            OneHttpUtil.getLinkMicStream(new HttpCallback() {
                @Override
    ...
    public void onAudienceSendLinkMicUrl(String uid, String uname, String playUrl) {
        ...
        if (mIsAnchor) {
            ...
            OneHttpUtil.linkMicShowVideo(uid, playUrl);
        }
...
//        if (((ChatBaseActivity) mContext).isGamePlaying()) {
//            ToastUtil.show(R.string.live_game_cannot_link_mic);
//            return;
//        }
//        if (((ChatBaseActivity) mContext).isLinkMicAnchor()) {
...
//        }
    ...
    private void checkLinkMicEnable() {
        OneHttpUtil.checkLinkMicEnable(mLiveUid, new HttpCallback() {
            @Override
            public void onSuccess(int code, String msg, String[] info) {
                if (code == 0) {
    ...
    private void audienceApplyLinkMic() {
        ...
        if (curTime - mLastApplyLinkMicTime < 11000) {//时间间隔11秒
            ...
            //请求权限
//            ProcessResultUtil processResultUtil = ((ChatBaseActivity) mContext).getProcessImageUtil();
...
//        if (((ChatAnchorActivity) mContext).isBgmPlaying()) {
...
//                    ((ChatAnchorActivity) mContext).stopBgm();
//                    mAcceptLinkMic = true;
//                    if (mLinkMicPopWindow != null) {
//                        mLinkMicPopWindow.dismiss();
//                    }
